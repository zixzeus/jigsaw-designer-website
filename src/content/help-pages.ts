...
const pages: Record<HelpSlug, Record<TierOneLocale, ArticleContent>> = {
  "getting-started": {
    en: article("en", {
      ...
      sections: [
        ...
        {id: "generate-save-export", title: "3. Generate, save, and export", paragraphs: ["Select a suitable boundary and slot template, configure generation parameters, and run Generate Jigsaw. Inspect the returned SVG group, save the .jigsawproject source document, and export SVG when ready."], note: `The current app exports SVG only. Free users receive ${PRODUCT_FACTS.freeGenerationLimit} generations; Premium unlocks unlimited generation and SVG export.`},
      ],
      related: related("en", ["jigsaw-generation", "vector-editing", "project-library"]),
    }),
    "zh-Hans": article("zh-Hans", {
      slug: "help/getting-started", eyebrow: "JigsawDesigner 帮助", title: "JigsawDesigner 快速开始", seoTitle: "JigsawDesigner 快速开始", seoDescription: "创建项目、绘制或导入 SVG、生成拼图切割线、保存可编辑项目并导出 SVG。", intro: "按照这条简短路径，从空白项目开始建立可编辑的拼图文档。界面适配 iPhone、iPad 和 Mac，项目会把 SVG 几何、模板与设置保存在一起。", highlights: ["创建本地项目", "使用八种编辑工具", "导入已有 SVG 几何", "生成并导出 SVG 切割线"],
      sections: [
        {id: "create-project", title: "1. 创建项目", paragraphs: ["在项目库中选择新建项目。新项目默认使用 800 × 600 画布、100 片目标和内置卡槽模板，生成前可调整。"], image: {src: "/my_projects-v1-6.webp", alt: "从 JigsawDesigner 项目库创建项目"}},
        ...
        {id: "generate-save-export", title: "3. 生成、保存并导出", paragraphs: ["选择边界和卡槽模板，设置生成参数并运行生成。检查返回的 SVG 分组，保存 .jigsawproject 源文档，需要时导出 SVG。"], note: `当前只导出 SVG。免费用户有 ${PRODUCT_FACTS.freeGenerationLimit} 次生成；Premium 解锁无限生成与 SVG 导出。`},
      ],
      related: related("zh-Hans", ["jigsaw-generation", "vector-editing", "project-library"]),
    }),
    "zh-Hant": article("zh-Hant", {
      slug: "help/getting-started", eyebrow: "JigsawDesigner 說明", title: "JigsawDesigner 快速開始", seoTitle: "JigsawDesigner 快速開始", seoDescription: "建立專案、繪製或匯入 SVG、產生拼圖切割線、儲存可編輯專案並匯出 SVG。", intro: "依照這條簡短路徑，從空白專案建立可編輯的拼圖文件。介面支援 iPhone、iPad 與 Mac，專案會將 SVG 幾何、範本與設定儲存在一起。", highlights: ["建立本機專案", "使用八種編輯工具", "匯入現有 SVG 幾何", "產生並匯出 SVG 切割線"],
      sections: [
        {id: "create-project", title: "1. 建立專案", paragraphs: ["在專案庫選擇新增專案。新專案預設使用 800 × 600 畫布、100 片目標與內建卡槽範本，產生前可調整。"], image: {src: "/my_projects-v1-6.webp", alt: "從 JigsawDesigner 專案庫建立專案"}},
        ...
        {id: "generate-save-export", title: "3. 產生、儲存並匯出", paragraphs: ["選擇邊界與卡槽範本，設定產生參數並執行。檢查回傳的 SVG 群組，儲存 .jigsawproject 來源文件，需要時匯出 SVG。"], note: `目前只匯出 SVG。免費使用者有 ${PRODUCT_FACTS.freeGenerationLimit} 次產生；Premium 解鎖無限產生與 SVG 匯出。`},
      ],
  ...
  "jigsaw-generation": {
    en: article("en", {
      slug: "help/jigsaw-generation", eyebrow: "JigsawDesigner Help", title: "Generate Jigsaw Puzzle Cutlines", seoTitle: "How to Generate Jigsaw Puzzle Cutlines", seoDescription: "Choose a closed boundary and slot template, set grid, target piece count, and distance parameters, then inspect the generated SVG.", intro: "Jigsaw generation combines your vector boundary, a slot-template path, and project parameters in the native C++ engine. The resulting SVG is imported back into the editor as a group.", highlights: ["Closed vector boundary", "Built-in or custom slot path", "Grid and target piece controls", "Editable generated SVG group"],
      sections: [
        {id: "input-geometry", title: "Prepare the input geometry", paragraphs: ["Use a closed outer boundary for the puzzle area. Keep the intended boundary visible and unlocked, and remove geometry that should not participate in generation."], image: {src: "/gen_step1-v1-6.webp", alt: "Puzzle boundary selected before generation"}},
        {id: "parameters", title: "Choose a template and parameters", paragraphs: ["Select a slot template and set grid size, target piece count, minimum slot distance, and maximum slot distance. These are algorithm inputs; they do not replace inspection of the output."], image: {src: "/gen_step2-v1-6.webp", alt: "Selecting a jigsaw slot template"}},
        {id: "run-and-inspect", title: "Run generation and inspect the SVG", paragraphs: ["Choose Generate Jigsaw and wait for the progress operation to finish. The generated SVG returns as a named group. Zoom into boundaries and connections, inspect Layers, and repeat with adjusted inputs if needed."], image: {src: "/gen_step4-v1-6.webp", alt: "Generated puzzle cutlines in the editor"}},
      ],
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "help/jigsaw-generation", eyebrow: "JigsawDesigner 帮助", title: "生成拼图切割线", seoTitle: "如何生成拼图切割线", seoDescription: "选择闭合边界和卡槽模板，设置网格、目标片数与距离参数，再检查生成的 SVG。", intro: "拼图生成会把矢量边界、卡槽模板路径和项目参数交给原生 C++ 引擎，结果以 SVG 分组回到编辑器。", highlights: ["闭合矢量边界", "内置或自定义卡槽路径", "网格与目标片数控制", "可编辑的 SVG 结果分组"],
      sections: [
        {id: "input-geometry", title: "准备输入几何", paragraphs: ["使用闭合外边界定义拼图区域。保持目标边界可见且未锁定，并移除不应参与生成的几何。"], image: {src: "/gen_step1-v1-6.webp", alt: "生成前选择拼图边界"}},
        {id: "parameters", title: "选择模板与参数", paragraphs: ["选择卡槽模板，并设置网格大小、目标片数、最小和最大卡槽距离。这些是算法输入，不能代替对输出的检查。"], image: {src: "/gen_step2-v1-6.webp", alt: "选择拼图卡槽模板"}},
        {id: "run-and-inspect", title: "运行生成并检查 SVG", paragraphs: ["运行生成并等待进度完成。生成结果会作为命名分组返回。放大检查边界和连接，查看图层，必要时调整输入后重试。"], image: {src: "/gen_step4-v1-6.webp", alt: "编辑器中的拼图切割线结果"}},
      ],
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "help/jigsaw-generation", eyebrow: "JigsawDesigner 說明", title: "產生拼圖切割線", seoTitle: "如何產生拼圖切割線", seoDescription: "選擇封閉邊界與卡槽範本，設定網格、目標片數與距離參數，再檢查產生的 SVG。", intro: "拼圖產生會把向量邊界、卡槽範本路徑與專案參數交給原生 C++ 引擎，結果以 SVG 群組回到編輯器。", highlights: ["封閉向量邊界", "內建或自訂卡槽路徑", "網格與目標片數控制", "可編輯的 SVG 結果群組"],
      sections: [
        {id: "input-geometry", title: "準備輸入幾何", paragraphs: ["使用封閉外邊界定義拼圖區域。保持目標邊界可見且未鎖定，並移除不應參與產生的幾何。"], image: {src: "/gen_step1-v1-6.webp", alt: "產生前選擇拼圖邊界"}},
        {id: "parameters", title: "選擇範本與參數", paragraphs: ["選擇卡槽範本，並設定網格大小、目標片數、最小與最大卡槽距離。這些是演算法輸入，不能取代對輸出的檢查。"], image: {src: "/gen_step2-v1-6.webp", alt: "選擇拼圖卡槽範本"}},
        {id: "run-and-inspect", title: "執行產生並檢查 SVG", paragraphs: ["執行產生並等待進度完成。結果會以命名群組回傳。放大檢查邊界與連接、查看圖層，必要時調整輸入後重試。"], image: {src: "/gen_step4-v1-6.webp", alt: "編輯器中的拼圖切割線結果"}},
      ],
  ...
  "svg-import-export": {
    en: article("en", {
      ...
      sections: [
        ...
        {id: "export", title: "Export a portable SVG", paragraphs: ["Use Export SVG to write the current document as SVG. The export includes document geometry, groups, transforms, and preserved resources supported by the writer."], note: "SVG is the current and only export format."},
      ],
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "help/svg-import-export", eyebrow: "JigsawDesigner 帮助", title: "导入与导出 SVG 文件", seoTitle: "JigsawDesigner SVG 导入与导出指南", seoDescription: "导入常见 SVG 几何与嵌入资源，保留可编辑项目，并从 JigsawDesigner 导出 SVG 文档。", intro: "SVG 用于把矢量几何带入 JigsawDesigner，或把完成的几何交给其他工作流。可编辑的 .jigsawproject 是另一种源项目格式，用于保留状态与资源。", highlights: ["导入常见 SVG 形状与分组", "保留嵌入图片和绘制资源", "保存可编辑 .jigsawproject", "仅导出 SVG"],
      sections: [
        {id: "import", title: "把 SVG 导入当前项目", paragraphs: ["使用导入 SVG 选择文件。常见路径、形状、文本、图片、分组、变换、裁剪路径和 use 引用可解析进文档；复杂 SVG 仍应检查结果。"], note: "远程 HTTP 图片、script 和 foreignObject 会被阻止。请嵌入必要图片，不要依赖远程 URL。"},
        {id: "project-package", title: "保存可编辑源项目", paragraphs: [".jigsawproject 包含 project.json、document.svg 和内容寻址资源，适合保留完整可编辑项目、设置与嵌入图片。"]},
        {id: "export", title: "导出便携 SVG", paragraphs: ["使用导出 SVG 写出当前文档，包括几何、分组、变换和写入器支持的保留资源。"], note: "当前唯一导出格式为 SVG。"},
      ],
  ...
  "vector-editing": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "help/vector-editing", eyebrow: "JigsawDesigner 帮助", title: "使用矢量工具编辑拼图几何", seoTitle: "JigsawDesigner 矢量编辑指南", seoDescription: "使用 Select、Point Edit、Pencil、Line、Rectangle、Circle、Polygon 和 Bezier，以及图层、分组、变换与撤销。", intro: "JigsawDesigner 提供八种聚焦工具来创建和调整 SVG 几何。选择状态、元素层级和可撤销命令始终作用于真实文档树。", highlights: ["选择与多选", "节点与 Bezier 控制柄编辑", "更平滑的 Pencil 路径", "缩放、旋转、分组、锁定和隐藏"],
      sections: [
        {id: "tools", title: "选择合适的工具", paragraphs: ["Select 移动和变换元素；Point Edit 修改节点与控制柄；Pencil 自由绘制；Line、Rectangle、Circle 和 Polygon 创建常见形状；Bezier 创建受控曲线。"], image: {src: "/toolbar-v1-6.webp", alt: "JigsawDesigner 工具栏中的八种矢量工具"}},
        {id: "selection", title: "选择并变换文档元素", paragraphs: ["点击选择，Shift 点击加入多选，并在嵌套内容中深入分组。画布手柄围绕选区中心缩放和旋转，Inspector 提供数值设置。"], bullets: ["缩放时锁定比例", "通过图层检查嵌套层级", "锁定或隐藏的祖先会影响编辑", "用撤销和重做恢复命令"]},
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "help/vector-editing", eyebrow: "JigsawDesigner 說明", title: "使用向量工具編輯拼圖幾何", seoTitle: "JigsawDesigner 向量編輯指南", seoDescription: "使用 Select、Point Edit、Pencil、Line、Rectangle、Circle、Polygon 與 Bezier，以及圖層、群組、變形與還原。", intro: "JigsawDesigner 提供八種專注工具來建立與調整 SVG 幾何。選取狀態、元素階層與可還原命令始終作用於真實文件樹。", highlights: ["選取與多選", "節點與 Bezier 控制把手編輯", "更平滑的 Pencil 路徑", "縮放、旋轉、群組、鎖定與隱藏"],
      sections: [
        {id: "tools", title: "選擇合適的工具", paragraphs: ["Select 移動與變形元素；Point Edit 修改節點與控制把手；Pencil 自由繪製；Line、Rectangle、Circle 與 Polygon 建立常見形狀；Bezier 建立受控曲線。"], image: {src: "/toolbar-v1-6.webp", alt: "JigsawDesigner 工具列中的八種向量工具"}},
        {id: "selection", title: "選取並變形文件元素", paragraphs: ["點按選取，Shift 點按加入多選，並在巢狀內容中深入群組。畫布把手圍繞選取中心縮放與旋轉，Inspector 提供數值設定。"], bullets: ["縮放時鎖定比例", "透過圖層檢查巢狀階層", "鎖定或隱藏的上層會影響編輯", "用還原與重做恢復命令"]},
  ...
  templates: {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "help/templates", eyebrow: "JigsawDesigner 帮助", title: "创建和管理卡槽模板", seoTitle: "JigsawDesigner 卡槽模板指南", seoDescription: "选择内置拼图卡槽模板，或绘制、导入、编辑、导出、持久保存和删除自定义开放路径模板。", intro: "卡槽模板定义拼图生成所用的连接路径。内置模板受保护；自定义模板可创建、更新、导出、持久保存和删除。", highlights: ["标准凸形", "标准凹形", "直线", "自定义 SVG 开放路径"],
      sections: [
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "help/templates", eyebrow: "JigsawDesigner 說明", title: "建立與管理卡槽範本", seoTitle: "JigsawDesigner 卡槽範本指南", seoDescription: "選擇內建拼圖卡槽範本，或繪製、匯入、編輯、匯出、持久儲存與刪除自訂開放路徑範本。", intro: "卡槽範本定義拼圖產生使用的連接路徑。內建範本受保護；自訂範本可建立、更新、匯出、持久儲存與刪除。", highlights: ["標準凸形", "標準凹形", "直線", "自訂 SVG 開放路徑"],
      sections: [
  ...
  "project-library": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "help/project-library", eyebrow: "JigsawDesigner 帮助", title: "管理项目、iCloud 同步与分享", seoTitle: "JigsawDesigner 项目库指南", seoDescription: "管理本地项目、可选 iCloud 同步、公共项目发布与导入，以及只读 CloudKit 分享链接。", intro: "项目库汇集我的项目、公共项目和通过 CloudKit 分享的项目。常规保存始终包含本地持久化；云功能在启用且可用时工作。", highlights: ["本地项目包", "可选 iCloud 同步", "公共项目发布与导入", "只读 CloudKit 分享链接"],
      sections: [
        {id: "my-projects", title: "我的项目", paragraphs: ["在我的项目中创建、打开、重命名和删除。常规保存先写入本地 .jigsawproject；项目选择云端且 iCloud 可用时，还会安排 CloudKit 同步。"], image: {src: "/my_projects-v1-6.webp", alt: "JigsawDesigner 项目库中的我的项目"}},
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "help/project-library", eyebrow: "JigsawDesigner 說明", title: "管理專案、iCloud 同步與分享", seoTitle: "JigsawDesigner 專案庫指南", seoDescription: "管理本機專案、可選 iCloud 同步、公開專案發佈與匯入，以及唯讀 CloudKit 分享連結。", intro: "專案庫彙整我的專案、公開專案與透過 CloudKit 分享的專案。一般儲存始終包含本機持久化；雲端功能在啟用且可用時運作。", highlights: ["本機專案套件", "可選 iCloud 同步", "公開專案發佈與匯入", "唯讀 CloudKit 分享連結"],
      sections: [
        {id: "my-projects", title: "我的專案", paragraphs: ["在我的專案建立、開啟、重新命名與刪除。一般儲存先寫入本機 .jigsawproject；專案選擇雲端且 iCloud 可用時，還會安排 CloudKit 同步。"], image: {src: "/my_projects-v1-6.webp", alt: "JigsawDesigner 專案庫中的我的專案"}},

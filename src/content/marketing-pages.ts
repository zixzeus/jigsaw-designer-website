...
const pages: Record<MarketingSlug, Record<TierOneLocale, ArticleContent>> = {
  "jigsaw-puzzle-generator": {
      ...
      sections: [
        {
          ...
          image: {src: "/gen_step1-v1-6.webp", alt: "Selecting a vector boundary for jigsaw generation"},
        },
        {
          ...
          image: {src: "/advanced_settings-v1-6.webp", alt: "Jigsaw generation parameter controls"},
        },
        {
          id: "edit-and-export",
    ...
    "zh-Hans": article("zh-Hans", {
      ...
      sections: [
        {id: "boundary-to-cutlines", title: "从矢量边界到拼图切割线", paragraphs: ["先准备拼图的外轮廓。你可以使用 Pencil、Bezier、直线、矩形、圆形或多边形工具绘制，也可以导入已有 SVG。", "生成器会把项目几何与所选卡槽模板交给原生 C++ 引擎，并把生成的 SVG 作为可编辑分组重新导入文档。"], image: {src: "/gen_step1-v1-6.webp", alt: "选择用于拼图生成的矢量边界"}},
        {id: "generation-controls", title: "控制生成图案的参数", paragraphs: ["网格大小与目标片数描述所需密度，卡槽距离控制连接轮廓的放置范围。算法结果可能随输入而变化，因此应检查实际生成的几何。"], bullets: ["网格大小控制生成网格", "目标片数用于引导拼图密度", "最小与最大卡槽距离约束连接位置", "所选模板提供卡槽路径"], image: {src: "/advanced_settings-v1-6.webp", alt: "拼图生成参数设置"}},
        {id: "edit-and-export", title: "检查、编辑并导出", paragraphs: ["通过图层、选择、点编辑、分组、画布内缩放旋转和 Inspector 数值继续调整结果。当前唯一导出格式为 SVG。", "若要打印或切割，请在下游软件中确认单位、尺寸、线条处理、材料与设备参数。"], note: "JigsawDesigner 提供可编辑的 SVG 几何，但不对特定设备、材料或制造流程做兼容性认证。"},
      ],
      faq: [
    ...
    "zh-Hant": article("zh-Hant", {
      ...
      sections: [
        {id: "boundary-to-cutlines", title: "從向量邊界到拼圖切割線", paragraphs: ["先準備拼圖的外輪廓。可使用 Pencil、Bezier、直線、矩形、圓形或多邊形工具繪製，也可匯入現有 SVG。", "產生器會把專案幾何與所選卡槽範本交給原生 C++ 引擎，並將產生的 SVG 以可編輯群組重新匯入文件。"], image: {src: "/gen_step1-v1-6.webp", alt: "選擇用於拼圖產生的向量邊界"}},
        {id: "generation-controls", title: "控制產生圖案的參數", paragraphs: ["網格大小與目標片數描述所需密度，卡槽距離控制連接輪廓的放置範圍。演算法結果可能隨輸入改變，因此應檢查實際幾何。"], bullets: ["網格大小控制產生網格", "目標片數引導拼圖密度", "最小與最大卡槽距離限制連接位置", "所選範本提供卡槽路徑"], image: {src: "/advanced_settings-v1-6.webp", alt: "拼圖產生參數設定"}},
        {id: "edit-and-export", title: "檢查、編輯並匯出", paragraphs: ["透過圖層、選取、節點編輯、群組、畫布縮放旋轉與 Inspector 數值調整結果。目前唯一匯出格式為 SVG。", "若要列印或切割，請在下游軟體確認單位、尺寸、線條處理、材料與設備參數。"], note: "JigsawDesigner 提供可編輯 SVG 幾何，但不對特定設備、材料或製造流程提供相容性認證。"},
      ],
      faq: [
  ...
  "jigsaw-dieline-generator": {
    en: article("en", {
      ...
      sections: [
        {id: "what-is-generated", title: "What the dieline generator creates", paragraphs: ["A dieline describes vector boundaries and internal cutlines; it is not a rendered photograph of a finished puzzle. JigsawDesigner uses the supplied boundary, slot path, grid, target piece count, and distance settings to construct SVG geometry.", "The generated result remains part of the document tree, preserving the ability to select, group, transform, and inspect it before export."]},
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "jigsaw-dieline-generator", eyebrow: "可编辑矢量切割线", title: "带 SVG 编辑功能的拼图刀模生成器", seoTitle: "拼图刀模生成器与 SVG 切割线编辑器", seoDescription: "根据矢量边界生成拼图刀模，自定义卡槽轮廓，编辑路径与图层，并将 SVG 交给下一步工作流。", intro: "把拼图结构创建为可编辑的矢量切割线。JigsawDesigner 将算法生成与绘图、点编辑、图层、变换、自定义卡槽模板和 SVG 导出结合在一起。", highlights: ["算法驱动的内部切割线", "可编辑的矢量分组与路径", "自定义连接轮廓", "导出 SVG 供下游使用"],
      sections: [
        {id: "what-is-generated", title: "刀模生成器会创建什么", paragraphs: ["刀模描述边界与内部切割线，并不是成品拼图的位图照片。JigsawDesigner 根据边界、卡槽路径、网格、目标片数和距离设置构建 SVG 几何。", "结果仍在文档树中，可在导出前选择、分组、变换和检查。"]},
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "jigsaw-dieline-generator", eyebrow: "可編輯向量切割線", title: "具備 SVG 編輯功能的拼圖刀模產生器", seoTitle: "拼圖刀模產生器與 SVG 切割線編輯器", seoDescription: "依向量邊界產生拼圖刀模、自訂卡槽輪廓、編輯路徑與圖層，並將 SVG 交給下一步工作流程。", intro: "把拼圖結構建立為可編輯的向量切割線。JigsawDesigner 結合演算法產生、繪圖、節點編輯、圖層、變形、自訂卡槽範本與 SVG 匯出。", highlights: ["演算法驅動的內部切割線", "可編輯的向量群組與路徑", "自訂連接輪廓", "匯出 SVG 供下游使用"],
      sections: [
        {id: "what-is-generated", title: "刀模產生器會建立什麼", paragraphs: ["刀模描述邊界與內部切割線，不是完成拼圖的點陣照片。JigsawDesigner 依邊界、卡槽路徑、網格、目標片數與距離設定建立 SVG 幾何。", "結果仍位於文件樹中，可在匯出前選取、群組、變形與檢查。"]},
  ...
  "laser-cut-jigsaw-puzzle-svg": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "laser-cut-jigsaw-puzzle-svg", eyebrow: "SVG 下游工作流", title: "为激光切割工作流准备拼图 SVG", seoTitle: "激光切割拼图 SVG 工作流", seoDescription: "生成并检查拼图 SVG，再在下游切割软件中验证尺寸、单位、描边、切缝、材料和设备参数。", intro: "JigsawDesigner 负责创建可编辑的 SVG 切割线几何。完成设计后，把导出的 SVG 放入实际切割软件，并按材料与设备要求设置生产参数。", highlights: ["生成可编辑 SVG 路径", "检查图层与连接", "导出标准 SVG 文档", "在下游验证全部生产参数"],
      sections: [
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "laser-cut-jigsaw-puzzle-svg", eyebrow: "SVG 下游工作流程", title: "為雷射切割工作流程準備拼圖 SVG", seoTitle: "雷射切割拼圖 SVG 工作流程", seoDescription: "產生並檢查拼圖 SVG，再於下游切割軟體驗證尺寸、單位、描邊、切縫、材料與設備參數。", intro: "JigsawDesigner 負責建立可編輯的 SVG 切割線幾何。完成設計後，將匯出的 SVG 放入實際切割軟體，並依材料與設備要求設定參數。", highlights: ["產生可編輯 SVG 路徑", "檢查圖層與連接", "匯出標準 SVG 文件", "在下游驗證全部生產參數"],
      sections: [
  ...
  "custom-jigsaw-puzzle-template": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "custom-jigsaw-puzzle-template", eyebrow: "可复用连接轮廓", title: "创建自定义拼图卡槽模板", seoTitle: "自定义拼图模板 SVG｜卡槽轮廓编辑", seoDescription: "在 JigsawDesigner 中创建、编辑、导入、导出并复用开放路径卡槽模板，用于拼图切割线生成。", intro: "卡槽模板向拼图生成器提供连接轮廓。你可以从三个内置轮廓开始，也可以创建符合自己设计语言的开放矢量路径。", highlights: ["三个内置起始模板", "基于 Bezier 的自定义路径", "导入与导出模板 SVG", "自定义模板持久保存"],
      sections: [
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "custom-jigsaw-puzzle-template", eyebrow: "可重複使用的連接輪廓", title: "建立自訂拼圖卡槽範本", seoTitle: "自訂拼圖範本 SVG｜卡槽輪廓編輯", seoDescription: "在 JigsawDesigner 中建立、編輯、匯入、匯出並重複使用開放路徑卡槽範本。", intro: "卡槽範本向拼圖產生器提供連接輪廓。可從三個內建輪廓開始，也可建立符合自己設計語言的開放向量路徑。", highlights: ["三個內建起始範本", "以 Bezier 建立自訂路徑", "匯入與匯出範本 SVG", "自訂範本持久儲存"],
      sections: [
  ...
  "svg-puzzle-editor": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "svg-puzzle-editor", eyebrow: "原生矢量编辑", title: "适用于 Mac、iPad 和 iPhone 的 SVG 拼图编辑器", seoTitle: "Mac、iPad 与 iPhone SVG 拼图编辑器", seoDescription: "在 Apple 设备上导入并编辑 SVG 拼图几何，使用绘图、点编辑、图层、分组、变换、真实缩放与 SVG 导出。", intro: "使用 Apple 平台原生编辑器处理拼图边界、模板与生成的切割线。从导入到导出，路径、常见 SVG 形状和嵌套分组都可以继续编辑。", highlights: ["八种绘图与编辑工具", "多选、分组和图层", "画布内缩放与旋转", "1% 到 3200% 真实缩放"],
      sections: [
        {id: "svg-import", title: "把 SVG 图稿带入可编辑文档", paragraphs: ["可导入路径、矩形、圆形、文本、图片和分组几何。1.6.0 提升了嵌入图片、渐变、图案和裁剪路径在编辑、保存与重新打开时的保留可靠性。"], note: "不会获取远程 HTTP 图片引用；请把必要资源嵌入 SVG 或项目。"},
        {id: "vector-tools", title: "绘制与编辑矢量几何", paragraphs: ["可使用 Select、Point Edit、Pencil、Line、Rectangle、Circle、Polygon 和 Bezier。Pencil 提供更平滑的自由绘制，开放轮廓可以继续或连接。"], bullets: ["Shift 多选元素", "深入嵌套分组", "编辑锚点与 Bezier 控制柄", "围绕选区中心缩放和旋转"]},
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "svg-puzzle-editor", eyebrow: "原生向量編輯", title: "適用於 Mac、iPad 與 iPhone 的 SVG 拼圖編輯器", seoTitle: "Mac、iPad 與 iPhone SVG 拼圖編輯器", seoDescription: "在 Apple 裝置上匯入並編輯 SVG 拼圖幾何，使用繪圖、節點編輯、圖層、群組、變形、真實縮放與 SVG 匯出。", intro: "使用 Apple 平台原生編輯器處理拼圖邊界、範本與產生的切割線。從匯入到匯出，路徑、常見 SVG 形狀與巢狀群組都能繼續編輯。", highlights: ["八種繪圖與編輯工具", "多選、群組與圖層", "畫布內縮放與旋轉", "1% 到 3200% 真實縮放"],
      sections: [
        {id: "svg-import", title: "把 SVG 圖稿帶入可編輯文件", paragraphs: ["可匯入路徑、矩形、圓形、文字、圖片與群組幾何。1.6.0 提升嵌入圖片、漸層、圖樣與裁剪路徑在編輯、儲存與重新開啟時的保留可靠性。"], note: "不會擷取遠端 HTTP 圖片引用；請將必要資源嵌入 SVG 或專案。"},
        {id: "vector-tools", title: "繪製與編輯向量幾何", paragraphs: ["可使用 Select、Point Edit、Pencil、Line、Rectangle、Circle、Polygon 與 Bezier。Pencil 提供更平滑的自由繪製，開放輪廓可繼續或連接。"], bullets: ["Shift 多選元素", "深入巢狀群組", "編輯錨點與 Bezier 控制把手", "圍繞選取中心縮放與旋轉"]},
  ...
  "how-to-make-a-laser-cut-jigsaw-puzzle": {
    ...
    "zh-Hans": article("zh-Hans", {
      slug: "how-to-make-a-laser-cut-jigsaw-puzzle", schemaType: "HowTo", eyebrow: "逐步矢量工作流", title: "如何为激光切割工作流制作拼图 SVG", seoTitle: "如何制作激光切割拼图 SVG", seoDescription: "从设计拼图边界、生成切割线、编辑 SVG，到在下游激光切割软件中验证文件的实用流程。", intro: "本指南覆盖激光切割拼图项目的矢量设计阶段。JigsawDesigner 创建并导出 SVG；生产参数与安全要求应以切割软件和设备文档为准。", highlights: ["创建或导入拼图边界", "选择并测试卡槽模板", "生成并检查切割线几何", "导出 SVG 后在下游验证"],
      sections: [
        {id: "prepare-boundary", title: "1. 准备外边界", paragraphs: ["创建项目并绘制闭合外轮廓，或导入合适的 SVG 边界。设置画布尺寸，确认目标几何可见且可选。"], image: {src: "/gen_step1-v1-6.webp", alt: "准备拼图生成使用的 SVG 边界"}},
        {id: "choose-and-generate", title: "2. 选择模板并生成", paragraphs: ["选择标准凸形、标准凹形、直线或自定义卡槽模板。设置网格大小、目标片数和卡槽距离，然后运行生成。"], image: {src: "/gen_step3-v1-6.webp", alt: "设置并运行拼图生成"}},
        {id: "inspect-edit", title: "3. 检查并编辑路径", paragraphs: ["放大检查边界和内部连接，用图层、选择、点编辑、分组、缩放与旋转调整矢量文档。导出前保存可编辑项目。"], bullets: ["检查意外重叠或重复路径", "确认文档比例", "仅保留下一步需要的几何", "必要时调整输入并重新生成"]},
        {id: "export-and-test", title: "4. 导出 SVG 并在切割软件中验证", paragraphs: ["导出 SVG 后，将其导入设备所用软件并分配正确操作。根据设备文档确认单位、描边解释、切缝、材料、速度、功率和安全流程，并先做小范围测试。"], note: "本指南不对特定设备或材料做认证；生产验证由使用者完成。"},
      ],
    ...
    "zh-Hant": article("zh-Hant", {
      slug: "how-to-make-a-laser-cut-jigsaw-puzzle", schemaType: "HowTo", eyebrow: "逐步向量工作流程", title: "如何為雷射切割工作流程製作拼圖 SVG", seoTitle: "如何製作雷射切割拼圖 SVG", seoDescription: "從設計拼圖邊界、產生切割線、編輯 SVG，到在下游雷射切割軟體驗證檔案的實用流程。", intro: "本指南涵蓋雷射切割拼圖專案的向量設計階段。JigsawDesigner 建立並匯出 SVG；生產參數與安全要求應以切割軟體和設備文件為準。", highlights: ["建立或匯入拼圖邊界", "選擇並測試卡槽範本", "產生並檢查切割線幾何", "匯出 SVG 後在下游驗證"],
      sections: [
        {id: "prepare-boundary", title: "1. 準備外邊界", paragraphs: ["建立專案並繪製封閉外輪廓，或匯入合適的 SVG 邊界。設定畫布尺寸，確認目標幾何可見且可選取。"], image: {src: "/gen_step1-v1-6.webp", alt: "準備拼圖產生使用的 SVG 邊界"}},
        {id: "choose-and-generate", title: "2. 選擇範本並產生", paragraphs: ["選擇標準凸形、標準凹形、直線或自訂卡槽範本。設定網格大小、目標片數與卡槽距離，然後執行產生。"], image: {src: "/gen_step3-v1-6.webp", alt: "設定並執行拼圖產生"}},
        {id: "inspect-edit", title: "3. 檢查並編輯路徑", paragraphs: ["放大檢查邊界與內部連接，使用圖層、選取、節點編輯、群組、縮放與旋轉調整向量文件。匯出前儲存可編輯專案。"], bullets: ["檢查意外重疊或重複路徑", "確認文件比例", "只保留下一步所需幾何", "必要時調整輸入並重新產生"]},
        {id: "export-and-test", title: "4. 匯出 SVG 並在切割軟體驗證", paragraphs: ["匯出 SVG 後，將其匯入設備使用的軟體並指定正確操作。依設備文件確認單位、描邊解讀、切縫、材料、速度、功率與安全流程，並先做小範圍測試。"], note: "本指南不對特定設備或材料提供認證；生產驗證由使用者完成。"},
      ],

    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="fixed w-full z-50 bg-background/80 backdrop-blur-md border-b border-border transition-all duration-300">
        <div className="container mx-auto px-6 md:px-12 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-8 text-sm font-medium">
            ...
            <Link href="/help" className="hover:text-primary transition-colors">{tNav('help')}</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-32 pb-12 md:pt-40 md:pb-16 text-center container mx-auto px-6">
        <h1 className="text-4xl md:text-5xl font-bold mb-6">
          {tSupport('hero.titlePart1')} <span className="text-primary">{tSupport('hero.titlePart2')}</span>
        </h1>
      ...
      <div className="container mx-auto px-6 md:px-12 py-12 max-w-5xl">
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-background border border-border p-8 rounded-2xl hover:shadow-lg transition-all">
            ...
            <h2 className="text-2xl font-bold mb-4">{tSupport('documentation.title')}</h2>
            <p className="text-gray-500 mb-6">
              {tSupport('documentation.description')}
            </p>
            <Link href="/help" className="text-primary font-semibold hover:underline flex items-center gap-1">
              {tSupport('documentation.action')} <span aria-hidden="true">&rarr;</span>
            </Link>
          </div>
          ...
          <div className="bg-background border border-border p-8 rounded-2xl hover:shadow-lg transition-all">
            ...
            <h2 className="text-2xl font-bold mb-4">{tSupport('email.title')}</h2>
            <p className="text-gray-500 mb-6">
              {tSupport('email.description')}
            </p>
      ...
      <div className="bg-background-secondary py-24 mt-12">
        <div className="container mx-auto px-6 md:px-12 max-w-4xl">
           <h2 className="text-3xl font-bold mb-12 text-center">{tSupport('faq.title')}</h2>
           
           <div className="space-y-6">
             <FaqItem 
               question={tSupport('faq.q1.question')}
               answer={tSupport('faq.q1.answer')}
             />
             <FaqItem 
               question={tSupport('faq.q2.question')}
               answer={tSupport('faq.q2.answer')}
             />
             <FaqItem 
               question={tSupport('faq.q3.question')}
               answer={tSupport('faq.q3.answer')}
             />
           </div>
        </div>
      </div>
